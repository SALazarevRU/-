...
Public Sub строчек(editBox As IRibbonControl, ByRef text)
    ...
    cellValue = ActiveSheet.Range("AN1").Value
    ' Проверяем, что ячейка не пустая и содержит число
    If Not IsEmpty(cellValue) And IsNumeric(cellValue) Then
        строк = CLng(cellValue)
        text = "   " & строк
    Else
        ' Если ячейка пустая или не число — возвращаем пустую строку
        text = ""
    End If
    
    Exit Sub  ' Нормальный выход из процедуры
ErrorHandler:
    ' При любой ошибке (включая недоступность листа, защиту и т.п.)
    text = ""  ' Гарантированно возвращаем пустое значение
End Sub
