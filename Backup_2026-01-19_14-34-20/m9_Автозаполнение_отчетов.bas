...
'   внимание! если в этом модуле нет описания этой функции и обращаешься к ней в другой модуль-пропиши Module5_ФункцияПроверки_Ореп_wb.IsBookOpen("C:\Users\.....
    fl = IsBookOpen("Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\Динамика 2026 Электрозаводская.xlsx")
    UserName1 = Environ("USERNAME")
    MsgBox "Сергей Александрович!" & vbNewLine & "Файл " & "Динамика 2026 Электрозаводская.xlsx'" & IIf(fl, " уже открыт", " НЕ ЗАНЯТ")
...
'   Кол_воКлаймов = Range("AX3")        'значение для переменной беру из листа [ппон] книги [Шапка_5.xlsm], или из:
    ...
    Set wb2 = Workbooks.Open("Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\Динамика 2026 Электрозаводская.xlsx")
    ...
    ИмяЛиста1 = (currentmonthname & " 2026")
    ИмяЛиста2 = ("учет " & currentmonthname & " 2026")
...
'    Range("AX3").ClearContents        'не важно
    ...
    If Not Intersect(activeCell, Range("B4:B108")) Is Nothing Then
        Cells(activeCell.Row, 5).Value = Кол_воКлаймов        'Целевая ячейка2
...
'    Range("AE11:AQ37").Select   ' выделил диапазон
    Workbooks("Динамика 2026 Электрозаводская.xlsx").Save        ' сохраняю изменения в книге
    Set wb2 = Nothing        'удаляю присвоенное значение переменной Workbook
    Workbooks("Динамика 2026 Электрозаводская.xlsx").Close SaveChanges:=False        'закрываю без сохранения wb [Динамика...xlsx]
    Application.ScreenUpdating = True        'включаю обновление экрана.
    
     MsgBox "The value of the target cell in the worksheet [" & ИмяЛиста1 & "] = " & Результат1 & _
       vbNewLine & "Значение целевой ячейки на листе [" & ИмяЛиста2 & "] = " & Результат2, _
       vbOKOnly, "Проверка заполнения ячеек на двух листах файла [Динамика...]"
    
    ' ОТКРОЮ ДИНАМИКУ:
    Workbooks.Open FileName:="Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\Динамика 2026 Электрозаводская.xlsx"
...
'    Range("AE11:AQ37").Select   ' выделил диапазон
    ...
    Application.ScreenUpdating = True        'включаю обновление экрана.
     ...
     MsgBox "The value of the target cell in the worksheet [январь 2026] = " & Результат1 & _
        vbNewLine & "Значение целевой ячейки на листе [учет январь 2026] = " & Результат2, vbOKOnly, _
...
'--------------------------------------------
    ...
    'With Application.FileDialog(msoFileDialogFolderPicker)
        ...
        .InitialFileName = "C:\Users\s.lazarev\Desktop\2_Быстроденьги_сканинг\СКАНЫ_за_ВЧЕРА\" '"назначаем первую папку отображения
        ...
        PapkaSkanovФКБ = .SelectedItems(1) 'считываем путь к папке
...
'   Application.ScreenUpdating чуть ниже
    ...
    folderPath = PapkaSkanovФКБ
    ' получаем характеристики папки = PapkaSkanov
    Set fso = CreateObject("Scripting.FileSystemObject")
    КоличСканов = fso.GetFolder(folderPath).Subfolders.Count
         MsgBox "Выбрана папка " & PapkaSkanovФКБ & vbNewLine & vbNewLine & "Как бэ сегодня папок со сканами = " & КоличСканов
    ...
    currentmonthname = LCase(Format(Date, "mmmm")) ' получаем полное название текущего месяца в нижнем регистре
    pathОтчета = "Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\ОТЧЕТ по клаймам 2026\Отчет по клаймам за январь 2026.xlsx" ' полное имя Отчета по клаймам
    pathОтчета_с_currentmonthname = "Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\ОТЧЕТ по клаймам 2026\" & "Отчет по клаймам за " & currentmonthname & " 2026.xlsx"
    ...
    For Each wBook In Workbooks
        If wBook.Name = sWBName Then
            Set wBook = Workbooks("Отчет по клаймам за " & currentmonthname & " 2026.xlsx")
...
'     If MsgBox("Протянуть сегодня вниз на (КоличСканов-1) ячеек?", vbYesNo) <> vbYes Then Exit Sub
    ...
    ' обновить все сводные таблицы в книге
      ActiveWorkbook.RefreshAll
            ...
            ActiveWorkbook.ActiveSheet.DisplayPageBreaks = True 'Снова включить разбиение на печатные страницы
...
'--------------------------------------------
    ...
    pathОтчета = "Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\Для руководителей\Операционная отчетность фабрики\2026\Отчет по фабрикe.New (Январь 2026).xlsx" ' полное имя Отчета по клаймам
    pathОтчета_с_currentmonthname = "Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\Для руководителей\Операционная отчетность фабрики\2026\" & "Отчет по фабрикe.New (" & currentmonthname & " 2025).xlsx"
...
'        MsgBox "Внимание! " & vbNewLine & "файл уже открыт, ждем 20 сек и снова проверяем"
        ...
        GoTo Проверка
    Else
    Set wBook = Workbooks.Open(FileName:="Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\Для руководителей\Операционная отчетность фабрики\2026\" & "Отчет по фабрикe.New (" & currentmonthname & " 2026).xlsx")  ' открыл
