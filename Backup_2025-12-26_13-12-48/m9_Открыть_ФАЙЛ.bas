...
'    задаю переменные:
    ...
    If fl Then ' если уже был открыт мною
        ...
        wBook.Windows(1).Activate 'вытягиваем на первый план
    Else
        Set wBook = Workbooks.Open(FileName:="Q:\Corporative\ОСВ\ОБМЕННИК\Фабрика\отдел архива длительного хранения\архив\Динамика\ОТЧЕТ по клаймам\Отчет по клаймам за декабрь 2025.xlsx")  ' открыл
    End If
