...
Option Explicit
Option Compare Text                           ' ЭТО НЕ ТА ВЕРСИЯ !!! СМ АВТОЗАПОЛНЕНИЕ ОТЧЕТОВ !!!!
...
'--------------------------------------------
            ...
            КоличСканов = Sheets("ппон").Range("D11").Value
            Debug.Print "КоличСканов = " & КоличСканов
...
'    pathОтчета_с_currentmonthname = "C:\Users\Хозяин\Desktop\" & "Отчет по клаймам за " & currentmonthname & " 2025.xlsx"
 ...
 '  конец кода проверки, открыт ли наш файл
    ...
    Worksheets("отчет за день").Select
                        ...
                        Application.ScreenUpdating = False 'Оключил ЭКРАН
...
'    Application.DisplayAlerts = False 'Оключил предупреждения
    ...
    Workbooks("Архив. Поиск первички  НСК.xlsx").Sheets("Клаймы").Delete  '   удаляю Sheets(Клаймы)
  
    Workbooks("Отчет по клаймам за " & currentmonthname & " 2025.xlsx").Activate
'    Workbooks("Отчет по клаймам за сентябрь 2025.xlsx").Activate 'Первого числа Октября - вышла ошибка
