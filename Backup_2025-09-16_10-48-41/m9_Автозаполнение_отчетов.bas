...
'''    End With
                                                   ...
                                                   '    СМЕНИ ДАТУ !!!
                                                   '    СМЕНИ ДАТУ !!!
    Sheets("4692").Range("T1:T4693").AutoFilter Field:=20, Criteria1:="09.09.2025" 'СТАВЛЮ фильтры на активном листе в 20-м столбце Т
...
'   вствка второй раз:
    ...
    ActiveSheet.Range("$A$1:$B$135").AutoFilter Field:=2, Criteria1:="3"  ''поставил фильтр в столбце В на 3
     With Worksheets("Клаймы").AutoFilter.Range
       Range("A" & .Offset(1, 0).SpecialCells(xlCellTypeVisible)(1).Row).Select 'Код для выбора первой видимой ячейки сразу после заголовка отфильтрованного столбца:
    End With
